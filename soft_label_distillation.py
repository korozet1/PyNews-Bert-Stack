...
def model2train(teacher_model, student_model, train_loader, dev_loader, num_epochs, learning_rate, device, save_path):
    """
    训练学生模型（BiLSTM）使用软标签蒸馏，结合教师模型（BERT）的概率分布和预测类别。

    参数：
        teacher_model: 教师模型（BERT），提供软标签和硬标签。
        student_model: 学生模型（BiLSTM），需要学习教师模型的知识。
    ...
    print(f"num_epochs: {num_epochs}, learning_rate: {learning_rate}, device: {device}, T: {T}, alpha: {alpha}, batch_size: {train_loader.batch_size}")
    ...
    for epoch in range(num_epochs):
        ...
        for batch in tqdm(train_loader, desc=f"Soft Label Distillation Epoch {epoch + 1}/{num_epochs}"):
            ...
            # 计算软标签损失（KL 散度）
            teacher_probs = F.softmax(teacher_logits / T, dim=1)  # 教师模型的概率分布
            student_probs = F.log_softmax(student_logits / T, dim=1)  # 学生模型的概率分布
            soft_loss = F.kl_div(student_probs, teacher_probs, reduction='batchmean') * (T * T)  # KL 散度损失
...
if __name__ == '__main__':
    ...
    device = conf.device
    save_path = conf.save_model_path3 + "/bilstm_soft_distill.pth"  # 自行定义保存路径
